...
app.post('/api/ats/score', upload.single('resume'), async (req, res) => {
  try {
    ...
    if (!resumeText.trim()) {
      return res.status(400).json({ error: 'Could not read text from the PDF resume.' });
    }
...
app.post('/api/portfolio/generate', (req, res) => {
  try {
    ...
    const html = generatePortfolioPage({
      ...
      resumeFileName: typeof resumeFileName === 'string' ? resumeFileName : 'resume.pdf',
      ctaPrimaryText: typeof ctaPrimaryText === 'string' ? ctaPrimaryText : '',
      ctaSecondaryText: typeof ctaSecondaryText === 'string' ? ctaSecondaryText : '',
