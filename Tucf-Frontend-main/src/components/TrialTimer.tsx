...
const TRIAL_NOTICE_SHOWN_KEY = 'tucf_trial_notice_shown';
...
const TrialTimer: React.FC<TrialTimerProps> = ({ inline = false }) => {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setIsActive(true);
      setShowNotice(localStorage.getItem(TRIAL_NOTICE_SHOWN_KEY) !== 'true');
  ...
  useEffect(() => {
    if (!isActive || localStorage.getItem(TRIAL_NOTICE_SHOWN_KEY) === 'true') {
      return;
    }

    const hideNotice = window.setTimeout(() => {
      localStorage.setItem(TRIAL_NOTICE_SHOWN_KEY, 'true');
      setShowNotice(false);
    }, 2000);

    return () => window.clearTimeout(hideNotice);
  }, [isActive]);
  ...
  if (!isActive || !showNotice || ['pro', 'starter', 'complete'].includes(plan)) {
    return null;
  }
