...
async function storedResumeToFile(resume: StoredResumeFile): Promise<File> {
  const response = await fetch(resume.dataUrl);
  const blob = await response.blob();
  return new File([blob], resume.name, { type: resume.type || 'application/pdf' });
}
...
const ATSScoring: React.FC = () => {
  ...
  const analyzeResume = async () => {
    if ((!resume && !storedResume) || !jobDescription.trim() || !user) {
      return;
    }
    ...
    try {
      const analysisFile = resume ?? (await storedResumeToFile(storedResume));
      const result = await scoreResume(analysisFile, jobDescription.trim());
          ...
          <div className={atsCardClass}>
            ...
            <div className="mt-4">
              <button
                ...
                disabled={(!resume && !storedResume) || !jobDescription.trim() || isAnalyzing}
          ...
          {analysisResult ? (
            <>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                {analysisResult.improvementPlan?.length ? (
                  <div className={`${atsCardClass} animate-[fadeIn_400ms_ease-out]`}>
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div>
                        <h3 className="text-lg font-semibold flex items-center" style={{ color: 'var(--text-primary)' }}>
                          <Lightbulb className="h-5 w-5 mr-2" style={{ color: 'var(--accent)' }} />
                          Highest-impact improvements
                        </h3>
                        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
                          Estimated gains are capped guidance, not guaranteed score increases. Add only truthful skills and evidence.
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                      {analysisResult.improvementPlan.map((item) => (
                        <article key={`${item.category}-${item.title}`} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 transition-all duration-300 hover:-translate-y-1 hover:border-orange-400/50 hover:bg-orange-400/[0.06]">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <span className="text-[11px] uppercase tracking-[0.16em]" style={{ color: 'var(--text-secondary)' }}>{item.category}</span>
                              <h4 className="font-semibold mt-1" style={{ color: 'var(--text-primary)' }}>{item.title}</h4>
                            </div>
                            <span className="rounded-full px-2 py-1 text-xs font-semibold whitespace-nowrap" style={{ color: 'var(--accent)', background: 'rgba(255,122,0,0.12)' }}>+{item.estimatedPoints} pts*</span>
                          </div>
                          <p className="text-sm mt-3" style={{ color: 'var(--text-secondary)' }}>{item.action}</p>
                          <div className="mt-3 h-1.5 rounded-full bg-white/10"><div className="h-1.5 rounded-full bg-orange-400" style={{ width: `${Math.min(item.estimatedPoints * 6, 100)}%` }} /></div>
                          <span className="text-[11px] mt-2 inline-block uppercase" style={{ color: item.priority === 'high' ? 'var(--accent)' : 'var(--text-secondary)' }}>{item.priority} priority</span>
                        </article>
                      ))}
                    </div>
                  </div>
                ...
                {analysisResult.formattingChecks?.length ? (
                  <div className={`${atsCardClass} animate-[fadeIn_500ms_ease-out]`}>
                    <h3 className="text-lg font-semibold flex items-center mb-3" style={{ color: 'var(--text-primary)' }}>
                      <LayoutTemplate className="h-5 w-5 mr-2" style={{ color: 'var(--accent)' }} />
                      ATS layout and wording checks
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {analysisResult.formattingChecks.map((check) => (
                        <div key={check.label} className="rounded-xl border border-white/10 bg-black/30 p-4">
                          <div className="flex items-center gap-2 font-medium" style={{ color: 'var(--text-primary)' }}>
                            {check.status === 'pass' ? <CheckCircle2 className="h-4 w-4 text-emerald-400" /> : <AlertCircle className="h-4 w-4" style={{ color: 'var(--accent)' }} />}
                            {check.label}
                          </div>
                          <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>{check.detail}</p>
                        </div>
                      ))}
                    </div>
                  </div>
