...
import { getGroqApiKey, getGroqModel, resolveGroqModel } from "../lib/groq";
...
const CHATBOT_TEXT_MODEL = getGroqModel();
...
async function chatbot_streamGroq(
  ...
  try {
    const resolvedModel = model === CHATBOT_VISION_MODEL ? model : await resolveGroqModel(model);
    ...
    if (!response.ok) {
      ...
      const rawMessage =
        ...
        if (response.status === 401 || response.status === 403) {
          onError("Groq API key was rejected. Generate a new key in Groq Console and save it in Settings.");
          return;
        }
