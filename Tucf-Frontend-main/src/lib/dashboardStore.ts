...
const toTasks = (value: unknown): DashboardTask[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  return value.filter((item): item is DashboardTask => {
    if (!isRecord(item)) {
      return false;
    }
...
const isAccentClass = (value: unknown): value is CalendarTask["accentClass"] =>
  value === "cyan" || value === "orange" || value === "purple";

const isPeriod = (value: unknown): value is CalendarTask["period"] =>
  value === "AM" || value === "PM";

const toCalendarTasks = (value: unknown): CalendarTask[] => {
  if (!Array.isArray(value)) {
    return defaultData().calendarTasks;
  }

  const parsed = value.filter((item): item is CalendarTask => {
    if (!isRecord(item)) {
      return false;
    }
    ...
    return (
      ...
      isAccentClass(item.accentClass)
    );
  });

  return parsed.length > 0 ? parsed : defaultData().calendarTasks;
};
...
export const readDashboardData = (user: DashboardUser): UserDashboardData => {
  ...
  const parsed = safeParseJSON<Partial<UserDashboardData>>(
    raw,
    "dashboard.store",
  );
  ...
  return {
    tasks: toTasks(parsed.tasks),
    calendarTasks: toCalendarTasks(parsed.calendarTasks),
...
export const writeDashboardData = (
  user: DashboardUser,
    ...
    Math.min(
      ...
      Math.round(
        (analysis.score +
          ...
          analysis.semanticSimilarity.score) /
          3,
      ),
    ),
  );
